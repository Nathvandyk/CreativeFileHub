...
import type { DriveInfo, ActivityEntry, AppSettings } from "./types";
import { formatBytes, formatRelativeTime, openInExplorer, copyToClipboard } from "./utils";
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!cancelled) setPaths(s.watched_paths);
      } catch { /* ignore */ }
    }
    load();
    ...
    <div className="h-screen w-screen p-2 text-zinc-100 font-sans select-none">
      <div className="h-full flex flex-col bg-zinc-900/95 border border-zinc-700 rounded-2xl shadow-2xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-4 flex flex-col gap-5">
          {/* Directories — click to copy the folder path */}
          <div>
            <p className="text-xs text-zinc-500 uppercase tracking-widest mb-2">Directories</p>
            {paths.length === 0 ? (
              <p className="text-xs text-zinc-600">No watched paths — add them in Applications</p>
            ) : (
              <div className="flex flex-col gap-1.5">
                {paths.map((p) => (
                  <button
                    key={p}
                    onClick={() => copyPath(p)}
                    onContextMenu={(e) => { e.preventDefault(); openInExplorer(p); }}
                    title={`${p}\n(click to copy · right-click to open)`}
                    className="flex items-center gap-2 bg-zinc-800/60 hover:bg-zinc-800 rounded-lg px-3 py-2 transition-colors text-left"
                  >
                    <span className="text-xs text-zinc-500 shrink-0">📋</span>
                    <span className="text-xs font-mono text-zinc-300 truncate flex-1">{p}</span>
                    {copied === p && <span className="text-[10px] text-green-400 shrink-0">Copied ✓</span>}
                  </button>
                ))}
              </div>
            )}
          ...
          {/* Recent projects — click to open the folder */}
          <div>
